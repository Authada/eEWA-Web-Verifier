...
export class OnlineAuthenticationSIOPService {
	...
	initTransaction (): Observable<PresentationDefinitionResponse> {
		...
		if (!this.deviceDetectorService.isDesktop()) {
			dataRequest['wallet_response_redirect_uri_template'] = this.httpService.apiUrl+'/get-wallet-code/?response_code={RESPONSE_CODE}';
		}
	...
	initCborTransaction (presentation: Presentation): Observable<PresentationDefinitionResponse> {
		const payload: any = {...presentation};
		payload.nonce = uuidv4();
		if (!this.deviceDetectorService.isDesktop()) {
			payload['wallet_response_redirect_uri_template'] = this.httpService.apiUrl + '/get-wallet-code?response_code={RESPONSE_CODE}';
		}
